...
import { Timezone, findTimezone, getLocalTimezone } from "@/data/timezones";
...
export default function TimezonePage() {
  ...
  const [sourceTimezone, setSourceTimezone, sourceHydrated] =
    useLocalStorage<Timezone>("tz_source", getLocalTimezone());
  const [targetTimezone, setTargetTimezone, targetHydrated] =
    useLocalStorage<Timezone>(
      "tz_target",
      findTimezone("asia/tokyo") || getLocalTimezone()
    );

  const [sourceTimezoneObj, setSourceTimezoneObj] =
    useState<Timezone>(sourceTimezone);
  const [targetTimezoneObj, setTargetTimezoneObj] =
    useState<Timezone>(targetTimezone);
      ...
      >
        ...
        >
          ...
          {error && (
            ...
            >
              <p style={{ color: isDark ? "#fca5a5" : "#dc2626", margin: 0 }}>
                {error}
              </p>
              <button
                onClick={() => setError("")}
                ...
                >
                  ...
                  >
                    ...
                    >
                      ...
                      {(conversionResult.dayDifference === "nextDay" &&
                        `(${t("results.nextDay")})`) ||
                        (conversionResult.dayDifference === "previousDay" &&
                          `(${t("results.previousDay")})`)}
                    </p>
                  </div>
